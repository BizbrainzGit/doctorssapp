...
AddPrescriptionView();   
        function AddPrescriptionView(){
            $.ajax({
                type  : 'GET',
                url   : url+"AddPrescriptionController/SearchAppointmentbookingList",
                async : true,
                dataType : 'json',
                headers:headers,
                success : function(result){
               if(result.success===true){
               AddPrescriptionListview(result.data,result.role);
...
function AddPrescriptionListview(addprescriptionListdata,role){
if ( $.fn.DataTable.isDataTable('#addprescriptiontable')) {
         $('#addprescriptiontable').DataTable().destroy();
         }  
         $('#addprescriptiontable tbody').empty();

         var data=addprescriptionListdata;
         var table = $('#addprescriptiontable').DataTable({
           ...
           mRender: function (data, type, row) {
            if(role=="Patient"){
             return  ''
               ...
               return '<button class="btn btn-primary btn-sm ShowPrescriptionAdd" id="ShowPrescriptionAdd" title="Add Prescription"><a data-appointmentbookingid="'+data.id+'" data-appointmentbookingname="' +data.name+ '" style="color:#ffffff"> <i class="mdi mdi-pencil-box-outline"></i> </a></button>&nbsp;'
...
$(document).on('click', '.ShowPrescriptionAdd a', function(e){
 ...
 $.ajax({
    type: "GET",
    url:url+'AddPrescriptionController/AppointmentDataForPrescription/'+id,
    dataType: 'json',
    headers:headers,
    success:function(result){
        if(result.success===true)
        { 
          $(".ReceptionistAddPrescriptionList-class").hide();
...
$("#addprescription").click(function() {
    ...
    if(!$("#add_prescriptiondata").valid())
   {
     return false;
      ...
      $.ajax({
      type:"POST",
      url:url+"AddPrescriptionController/savePrescriptionData",
      dataType: 'json',
      headers:headers,
      data:formData,
      contentType: false, 
